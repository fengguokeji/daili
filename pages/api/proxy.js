// pages/api/proxy.js
import https from 'https'; // 1. 引入 https 模块
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const response = await fetch(targetUrl, {
      ...
      agent: agent,       // 3. 将 agent 传入 fetch (针对 node-fetch)
      // 如果是 Node 18+ 原生 fetch，agent 可能不生效，需要下面这行粗暴的全局设置：
      // process.env.NODE_TLS_REJECT_UNAUTHORIZED = '0'; 
    });
    
    // 4. (可选兜底) 如果上面的 agent 没生效，还是报错，请取消注释下面这行：
    ...
    // 5. 打印详细错误，如果是 SSL 问题，下面的全局变量可能需要开启
    console.error(err);
